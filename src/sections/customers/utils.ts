...
type Props = {
  inputData: CustomersTableRowProps[];
...
export function applyFilter({ inputData, comparator, filterName, filterType, filterStatus }: Props) {
  ...
  if (filterName) {
    inputData = inputData.filter(
      (item) => item.name.toLowerCase().indexOf(filterName.toLowerCase()) !== -1 ||
                ...
                (item.company && item.company.toLowerCase().indexOf(filterName.toLowerCase()) !== -1)
    );
  }

  if (filterType !== 'all') {
    inputData = inputData.filter((item) => item.customerType === filterType);
  }
