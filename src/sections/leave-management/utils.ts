...
type Props = {
  inputData: LeaveManagementTableRowProps[];
...
export function applyFilter({ inputData, comparator, filterName, filterLeaveType, filterStatus }: Props) {
  ...
  if (filterLeaveType !== 'all') {
    inputData = inputData.filter((item) => item.leaveType === filterLeaveType);
  }
