...
type Props = {
  inputData: EmployeesTableRowProps[];
...
export function applyFilter({ inputData, comparator, filterName, filterDepartment, filterStatus }: Props) {
  ...
  if (filterName) {
    inputData = inputData.filter(
      (item) => item.name.toLowerCase().indexOf(filterName.toLowerCase()) !== -1 ||
                ...
                item.employeeId.toLowerCase().indexOf(filterName.toLowerCase()) !== -1
    );
  }

  if (filterDepartment !== 'all') {
    inputData = inputData.filter((item) => item.department === filterDepartment);
  }
